...
def objective(trial):
        ...
        L1 = trial.suggest_int("L1", 16, 128, 4, False)
        L2 = trial.suggest_int("L2", 16, 128, 4, False)
        look_back = trial.suggest_int("look_back", 4, 80, 4, False)
        ...
        model = Sequential()
        model.add(LSTM(L1, input_shape=(look_back, 1), return_sequences=True,stateful=True))
        # ,stateful=True
        #model.add(LSTM(5, batch_input_shape=(batch_size, look_back, 1),  return_sequences=True))
        model.add(Dropout(0.2))
        model.add(LSTM(L2,stateful=True ))
        model.add(Dropout(0.2))
        model.add(Dense(1))
        ...
        hist=model.fit(trainX,trainY, batch_size=32,epochs=80, callbacks=[custom_early_stopping],
...
def anal_objective(trial):
        ...
        trainX, trainY = create_shape(train,look_back)
        valX, valY = create_shape(val,look_back)
        testX, testY = create_shape(val,look_back)
        ...
        joblib.dump([trainScore, valScore, testScore, trainPredict, valPredict, testPredict],"study.pkl")
        !mkdir -p saved_model
