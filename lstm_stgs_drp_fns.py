...
[look_back,batch_size,L1]=[4,64,4]
...
def stg4_nodrp():
        ...
        hist=model.fit(trainX,trainY, batch_size=batch_size,epochs=3, callbacks=[custom_early_stopping],
...
def stg1_nodrp():
        model = Sequential()
        # model.add(Masking(mask_value=-5, input_shape=(look_back, 1)))
        model.add(LSTM(L1, input_shape=(look_back, 1)))
        model.add(Dense(1))
        model.compile(loss='mean_squared_error', optimizer='adam')
        custom_early_stopping = EarlyStopping(
            monitor='val_loss', 
